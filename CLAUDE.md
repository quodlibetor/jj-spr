...
- `spr.stackDisplay` - How a PR says which stack it belongs to: `section` (default) writes a `Stack` list into the PR body, `github` registers the PRs with GitHub's Stacked Pull Requests API so GitHub draws the stack, `none` says nothing. One setting rather than two flags, so the two descriptions cannot both be on and disagree. `github` requires `spr.baseStrategy = linear`, which it supplies when the strategy is unset and refuses to override when it is set to `synthetic`. Changing a stacked PR's base means dissolving the whole stack first — GitHub rejects any base change while a PR is stacked — so a run that retargets anything mints a new stack number and leaves any member it did not push unstacked. `jj spr land` dissolves the stack holding the PR before it merges anything — GitHub refuses to merge a PR a stack holds, and its own stack merge destroys the PRs above — and dissolves any stack holding a PR it retargets afterwards; run `jj spr diff` to register what is left. `jj spr close` does not know about stacks yet: it retargets the PRs above the one it closes, so the stack has to be taken apart by hand first
